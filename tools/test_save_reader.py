...
from src.data.save_reader import find_save_files, read_save
...
for cat in save.cats[:50]:
    ...
    print(f"{cat.db_key:4d} {cat.name:22s} {cat.level:3d} {cat.age:4d} {cat.gender:>6s} {cat.active_class:12s} "
...
print("\n\n=== VALIDATION: Known cats from OCR fixtures ===\n")
known_cats = {
...
for cat in save.cats:
    if cat.name in known_cats:
        print(f"{cat.name}:")
        print(f"  Level={cat.level}, Age={cat.age}, Gender={cat.gender}, Class={cat.active_class}")
        print(f"  Stats: STR={cat.base_str} DEX={cat.base_dex} CON={cat.base_con} "
